/* Подключение заголовочных файлов с описаниями базовых функций windows и Direct3D */
#include <windows.h>
#include <windowsx.h>
#include <d3d9.h>
#include <d3dx9.h>
...
/* Прототипы функций */
void initD3D(HWND hWnd);    /* Функция настройки и инициализации Direct3D */
...
typedef struct {
    ...
    D3DXVECTOR3 norm;    // Вектор нормали вершины
...
int WINAPI WinMain(HINSTANCE hInstance,
    ...
    /* Создание главного окна приложения по зарегистрированному классу */
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    switch(message) {
        ...
        case WM_MOUSEMOVE:
            ...
            if (mouse_r) {
                /* Изменение приближения/отдаления */
                fov += dx/10.0f;
            }
            /* Сохранение положения курсора */
            ox = x;
...
/* Функция настройки и инициализации Direct3D */
void initD3D(HWND hWnd) {
    d3d = Direct3DCreate9(D3D_SDK_VERSION);    // создание интерфейса D3D
    D3DPRESENT_PARAMETERS d3dpp;    // Создание структуры информации устройства
...
void initLight(void) {
    D3DLIGHT9 light;    // Создание структуры освещения
    D3DMATERIAL9 material;    // Создание структуры материала
    ZeroMemory(&light, sizeof(light));    // Очистка структуры освещения
    light.Type = D3DLIGHT_DIRECTIONAL;    // Установка освещения типа 'направленный свет'
...
void initGraphics(void) {
    ...
    // Блокировка вершинного буфера и загрузка в него вершин
    vBuffer->Lock(0, 0, (void**)&pVoid, 0);
    memcpy(pVoid, vertices, sizeof(vertices));
    // Разблокировка вершинного буфера
    vBuffer->Unlock();
    // Загрузка текстур
...
void viewTransform() {
    D3DXMATRIX matView;    // Матрица трансформации представления
    ...
    D3DXMatrixPerspectiveFovLH(&matProjection,
                   D3DXToRadian(fov),    // Угол поля обзора
...
void cleanD3D(void) {
    vBuffer->Release();    // Закрытие и освобождение вершинного буфера
